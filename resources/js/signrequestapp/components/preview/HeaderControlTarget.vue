<template>
  <preview-control>

    <div slot="content" class="py-2 ">
      <b-btn size="sm" variant="secondary" @click="handleCancelClick">
        <i class="fa fa-close"></i>
        <b>
          {{'controlTarget.cancel' | trans}}

        </b>
      </b-btn>
      <b-btn size="sm" variant="c-white" @click="handleResetClick">
        <i class="fa fa-refresh"></i>
        <b>
          {{'controlTarget.reset' | trans}}

        </b>
      </b-btn>
      <b-btn size="sm" variant="warning " @click="handleApplyClick">
        <i class="fa fa-check"></i>
        <b>
          {{'controlTarget.apply' | trans}}

        </b>
      </b-btn>
    </div>
  </preview-control>
</template>
<script>
  import {
    mapState,
    mapActions,
    mapMutations
  } from 'vuex'
  import PreviewControl from '../slots/PreviewControl'
  export default {
    name: 'PreviewHeaderControlTarget',
    components: {
      PreviewControl
    },
    data: function() {
      return {}
    },
    created() {},
    computed: mapState({
      parentPage: state => state.parentPage,
    }),
    methods: {
      ...mapActions(['setDataPdf', 'setParentPage', 'setStep', 'setFileUploaded', 'setLoading', 'setPdfTotalPage', 'setPdfCurrentPage', 'setPdfLoadedRatio', 'setCurrentSignArea', 'refreshDraggerInstance']),
      ...mapMutations(['ADDSIGNAREA', 'REMOVESIGNER']),
      bpValue(lgUpValue, lgDownValue) {
        return ['lg', 'xl'].includes(this.$mq) ? lgUpValue : lgDownValue
      },
      handleCancelClick() {
        this.setStep(2)
        this.REMOVESIGNER()
        this.$bvModal.show('signerFormModal')
      },
      handleResetClick() {
        let minWidth = this.parentPage.width * 0.3
        let minHeight = this.parentPage.width * 0.3 * 0.5
        let dragger = {
          width: minWidth,
          height: minHeight,
          top: 0,
          left: 0,
          elementTop: minHeight / 2,
          elementLeft: minWidth / 2
        }
        this.setCurrentSignArea(dragger)
        this.refreshDraggerInstance()

      },
      handleApplyClick() {
        this.ADDSIGNAREA()
        this.$bvModal.show('requesterFormModal')

        // this.setStep(4)
      },
    },
  }
</script>
<style>
</style>
